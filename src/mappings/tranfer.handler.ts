...
export async function erc721handleTransfer(
  ...
  if (!contractData) {
    if (oldOwner.id === NULL_ADDRESS) {
      ...
      ERC721contracts.addToUriUpdatedBuffer(contractData)
    } else {
  ...
  token.owner = owner
  if (oldOwner.id === NULL_ADDRESS) ERC721tokens.addToUriUpdatedBuffer(token)
  ERC721tokens.save(token)

  const transferId = transaction.hash
    .concat('-'.concat(tokenId))
    .concat('-'.concat(evmLog.index.toString()))
